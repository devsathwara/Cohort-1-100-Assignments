...
router.post('/signup', (req, res) => {
    try {
        ...
        if(username && password){
            const accessToken = jwt.sign({ username,password }, config.secret, { expiresIn: '1h' });
            const admin = new Admin({username:username,password:password});
            admin.save().then(()=>{
             return res.status(201).json({token:accessToken,message:'Admin created successfully'});
            });
        }
        else{
